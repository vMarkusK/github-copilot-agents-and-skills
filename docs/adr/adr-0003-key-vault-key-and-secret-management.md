...
- Follow least-privilege principles separately for key, secret, and certificate access
...
- Separate data-plane permissions by object type. Identities that need secrets must not automatically receive permissions for keys or certificates, and identities that perform cryptographic operations with keys must not automatically receive secret read access.
- Prefer object-type-specific built-in RBAC roles such as `Key Vault Secrets User`, `Key Vault Secrets Officer`, `Key Vault Crypto User`, `Key Vault Crypto Officer`, `Key Vault Certificates Officer`, and `Key Vault Certificate User` over broad administrative roles. Use `Key Vault Administrator` only for tightly controlled platform operations.
- Grant certificate management permissions only to dedicated operational identities or certificate automation workflows. Application runtime identities must not be granted certificate management unless the workload explicitly requires certificate lifecycle control.
...
- Separating key, secret, and certificate permissions reduces blast radius if an application or identity is compromised.
- Object-type-specific roles make authorization intent auditable and prevent privilege creep toward broad vault administration.
...
- **POS-003**: Separating permissions for keys, secrets, and certificates limits lateral privilege escalation between data-plane object types.
- **POS-004**: Premium Key Vault tier enables HSM-protected keys, rotation policy, and enterprise auditing capabilities.
- **POS-005**: Purge protection and soft delete prevent accidental or malicious permanent deletion.
- **POS-006**: Network restrictions and diagnostics support strengthen both confidentiality and operational monitoring.
- **POS-007**: Expiration and rotation policies enforce regular credential refresh and reduce long-lived secret usage.
...
- **NEG-003**: Fine-grained role separation for keys, secrets, and certificates increases the number of RBAC assignments that must be designed, reviewed, and maintained.
- **NEG-004**: Strict network and lifecycle policies may require additional support for legacy clients or ad-hoc access scenarios.
- **NEG-005**: Ephemeral secret handling may require application changes for dynamic secret retrieval and rotation handling.
- **NEG-006**: Automated key rotation may increase the need for integration testing to verify consumers can handle rotated keys.
...
- **IMP-005**: Set `rbac_authorization_enabled = true`, disable `access_policy` blocks, and use `azurerm_role_assignment` to grant object-type-specific least-privilege roles to managed identities.
- **IMP-006**: Model RBAC assignments separately for secrets, keys, and certificates. For example, application runtime identities should typically receive `Key Vault Secrets User` for secret retrieval, encryption-integrated services should receive `Key Vault Crypto Service Encryption User` or `Key Vault Crypto User`, and certificate automation identities should receive `Key Vault Certificates Officer` only when certificate lifecycle operations are required.
- **IMP-007**: Reserve `Key Vault Administrator` for break-glass or tightly controlled platform administration, and require justification and review before assigning any cross-object administrative role.
- **IMP-008**: Add review checks in Terraform code review or policy validation to reject role assignments that grant secret, key, and certificate management broadly without documented justification.
- **IMP-009**: Create HSM-backed keys via `azurerm_key_vault_key` with `key_type = "RSA-HSM"`, `key_size = 4096`, and `key_opts = ["decrypt", "encrypt", "sign", "unwrapKey", "verify", "wrapKey"]`.
- **IMP-010**: Set `expires_on = timestampadd("day", 365, utcnow())` for keys and `expiration_date` for secrets at creation time, using deployment-time logic to calculate one-year expiry.
- **IMP-011**: Configure rotation policy using `time_before_expiry` with `automatic { time_before_expiry = "P90D" }`, `expire_after = "P365D"`, and `notify_before_expiry = "P89D"`.
- **IMP-012**: Set `content_type` for all secrets and use descriptive values such as `"application/vnd.microsoft.keyvault-secret"`, `"password"`, or `"connection-string"` based on intent.
- **IMP-013**: Use ephemeral secret provisioning patterns where possible, such as temporary secrets created by deployment pipelines or application bootstrap flows, rather than long-lived static values.
- **IMP-014**: Document Key Vault operational processes, including identity onboarding, private endpoint usage, secret lifecycle, and key rotation validation.
